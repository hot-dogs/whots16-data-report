...
the temperature differences were in the {math}`±1.0 × 10^{-3}°C` range.
...
```{figure} figures/microcats/w16tcompare_1.png
:height: 1000px
:align: center
:name: figure5.2
...
panel), and 7215 (bottom panel), during the WHOTS-16 deployment. The light blue
...
VMCM and the 7-m MicroCAT temperatures during WHOTS-16, after adding a 0.0259
°C offset correction to the VMCM. The offset was the mean difference between
...
adjacent MicroCATs at 25 and 35-m during WHOTS-16 are shown in
...
adjacent MicroCATs at 45 and 50-m during WHOTS-16 are shown in
{numref}`figure5.5`. The ADCP failed and stopped collecting data on January
21, 2020 (see {ref}`/3_section.md#description-of-whots-16-mooring`). For 
...
adjacent MicroCATs at 120 and 135-m during WHOTS-16 are shown in
...
between the 120-m and the 135-m MicroCATs (lower panel) during the WHOTS-16
...
conductivity sensors during WHOTS-16 are shown in {numref}`figure5.8`
...
```{figure} figures/microcats/plt_w16_deep_corr.png
:height: 1000px
:align: center
:name: figure5.7

Temperature differences (top panel) and salinity differences (bottom panel)
between MicroCATs #12241 and #11391 during WHOTS-16. The blue (red) lines are
...
```{figure} figures/microcats/w16mic_corr8.jpg
:height: 1000px
:align: center
:name: figure5.15
...
Two TRDI broadband Workhorse Sentinel ADCP’s were deployed on the WHOTS-16
...
```{table} Specifications of the ADCP’s used for the WHOTS-16 mooring.
:class: sd-m-auto
:align: center
:name: table-16
...
Before the WHOTS-16 deployment, field calibration of the internal ADCP compass
...
After the WHOTS-16 mooring was recovered, the ADCP compass's performance was
...
pre-deployment calibration. Results from the WHOTS-16 post-deployment ADCP
...
```{figure} figures/adcp_moored/adcp_whot16cmpserr_sn4891.png
:height: 600px
:align: center
:name: figure5.16
...
```{figure} figures/adcp_moored/adcp_whot16cmpserr_sn1825.png
:height: 600px
:align: center
:name: figure5.17
...
Individual configurations for the two ADCP’s on the WHOTS-16 mooring are
detailed in {ref}`/appendices.md#whots-16-300-khz-serial-4891`, and
{ref}`/appendices.md#whots-16-600-khz-serial-1825`. The salient differences for
...
Temperature record from the 300 kHz ADCP during WHOTS-16 mooring (top panel).
...
```{table} ADCP record times (UTC mm/dd/yyyy, hh:mm:ss) during WHOTS-16 deployment
:class: sd-m-auto
:align: center
:name: table-21
...
in declination is small, approximately {math}`-0.02° year^{-1}` at the WHOTS 
location. A heading bias of 9.54° was entered in the setup of the WHOTS-16 
ADCP’s.
...
Sound speed profile (top panel) during the deployment of the WHOTS-16 mooring
from 2 dbar CTD data taken during regular HOT cruises and CTD profiles taken
during the WHOTS-16 and -17 deployment cruises (individual casts marked with a
...
procedures were applied to the WHOTS-16 deployment of ADCP data.
...
4. An indication of data quality for each ensemble is given by the `“percent
   ...
   {ref}`/appendices.md#whots-16-300-khz-serial-4891`). Data were flagged when
...
6. Histograms of raw vertical velocity data and partially cleaned data from the
   ...
   {ref}`/3_section.md#description-of-whots-16-mooring` , and therefore are not
...
Vector measuring current meters (VMCM) were deployed on the WHOTS-16 mooring at
...
A comparison of 30 m VMCM and ADCP U velocity for WHOTS-16. The top panel shows
...
```{figure} figures/ngvm_adcp/wh16_NGVM_30_V.png
:height: 1000px
:align: center
:name: figure5.25
...
```{figure} figures/ngvm_adcp/wh16_NGVM_10_V.png
:height: 1000px
:align: center
:name: figure5.27
...
top during the WHOTS-16 deployment 
({ref}`/3_section.md#description-of-whots-16-mooring`). Data returns from the
receiver were high ({numref}`table-23`). There was no ARGOS receiver for 
WHOTS-16.

```{table} GPS record times (UTC mm/dd/yy hh:mm) during WHOTS-16
:class: sd-m-auto
:align: center
:name: table-23
